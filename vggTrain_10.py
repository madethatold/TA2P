...
def generateSpecializedData(fileName, target, count1, count2):
    ...
    for index in range(1,6):
        cifar_10_path = fileName+"\data_batch_" + str(index)
        train = unpickle(cifar_10_path)
        x_temp = train[b'data']
        y_temp = train[b'labels']
        y_temp = np.asarray(y_temp)
        x_train_temp.append(x_temp)
        y_train_temp.append(y_temp)
    ...
    sp_y = keras.utils.to_categorical(sp_y, 10)
    sp_y = sp_y.astype('float32')
    sp_x = sp_x.astype('float32')
    return sp_x, sp_y

def generateSpecializedData_forTest(fileName, target, count1, count2):
    ...
    with tf.Session(graph=graph, config=config) as session:
        ...
        for epoch in range(1, 125 + 1):
            ...
            if epoch == 150 or epoch == 225:
                saver.save(session, 'vggNet_10/augmentation.ckpt', global_step=epoch)
            elif epoch % 10 == 0:
                saver.save(session, 'vggNet_10/augmentation.ckpt', global_step=epoch)
            if epoch % 25 == 0:
...
def run():
    label_count = 10
    targetList = range(10)
    train_data, train_labels = generateSpecializedData('D:\lifeng\workSpace\CDRP_SourceCode\cifar-10-batches-py',
                                                       targetList, 5000, 0)
    test_data, test_labels = generateSpecializedData_forTest('D:\lifeng\workSpace\CDRP_SourceCode\cifar-10-batches-py/test_batch',
                                                     targetList, 1000, 0)
