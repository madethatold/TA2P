...
def loss_(fusion_res, class_list):
    model_trim = TrimmedModel(target_class_id=class_list, multiPruning=True, pr=0.75)
...
def fitness_function_old(net, indexes):
    fusion_list = np.array([get_gatesAll_classId(indexes[0])
                               ...
                               , get_gatesAll_classId(indexes[4])
                            ]).T
...
#     net.save_model('./other/fusion_5/PSO/new/fusionNet' + ('_'.join(str(x) for x in class_indexes)) + '.pth')
...
#     return numbers_list
#
# directory = './other/fusion_2'  # 替换为你的目录路径
...
dict_res = {}
samples = [[0, 6], [2, 5], [2, 7], [2, 8], [3, 4], [3, 5], [3, 7], [4, 6], [4, 7], [4, 8], [6, 7], [6, 8], [7, 8]]
...
for i in tqdm(range(len(samples))):
    ...
    path_fusion = './other/fusion_2/fusionNet' + ('_'.join(str(x) for x in sample_indexes)) + '.pth'
...
# with open('fusion3_82_PSO.pkl', 'wb') as f:
