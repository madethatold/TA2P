...
class Model():
    ...
    def compute_encoding(self, data_input):
        ...
        graph = tf.Graph()  # 创建新的计算图
        self.build_model(graph, 10)
    ...
    def encode_class_data(self, class_id, train_images):
        for i in tqdm(range(len(train_images))):
            ...
            jsonpath = "./ImageEncoding_10/" + picname + ".json"
    ...
    def build_model(self, graph, label_count):
        with graph.as_default():
            ...
            gates_std = tf.add_n([tf.reduce_mean(tf.square(w-tf.reduce_mean(w))) for w in self.AllGateVariableValues])


            self.cross_entropy = ce
            self.l1_loss = self.penalty * l1_loss
    ...
    #     model_dir="other/vggNet/augmentation.ckpt-120" #checkpoint的文件位置
    ...
    def gate_variable(self, length, name='gate'):
        # 这里的length就是某一层的out_features
        initial = tf.constant([1.0] * length)  # 得到一个[1 1 1 1 1 .....]的tensor
    ...
    def conv2d(self, input, in_features, out_features, kernel_size, with_bias=False):
        ...
        gate = self.gate_variable(out_features)  # 卷积操作要乘的gate
    ...
    def batch_activ_fc_without_gate(self, current, in_features, out_features, is_training):
        Wfc = self.weight_variable_xavier([in_features, out_features], name='W')
        bfc = self.bias_variable([out_features])
        current = tf.matmul(current, Wfc) + bfc
        current = tf.contrib.layers.batch_norm(current, scale=True, is_training=is_training, updates_collections=None,
