...
class TrimmedModel:
    ...
    def __init__(self, target_class_id=[0], multiPruning=False, pr=0.5):
    ...
    def train_model(self, input_images, input_labels):
        if self.epoch == 25: self.learning_rate /= 10
    ...
    def mask_class_unit(self, classid):
        self.test_counter = 0
        theshold = 10
        json_path = "./ClassEncoding_10/class" + str(classid) + ".json"
    ...
    def mask_unit_by_value(self, classid):
        ...
        with open(json_path, "r") as f:
            ...
            for idx in range(len(gatesValueDict)):
                ...
                layer["name"] = name
                layer["shape"] = vec
            # with open("./ClassEncoding_10/mask1.json", "w") as tmpFile:
    ...
    def build_model(self, graph, label_count=10):
...
def fusionRes_to_gate(fusionRes):
    jsonpath = "./ClassEncoding_10/class1.json"
    res = []
    count = 0
...
#     model_trim = TrimmedModel(target_class_id=class_list, multiPruning=True, pr=0.85)
#
#     images, labels = d.test.generateSpecializedData_from_classes(class_list, 3000)
#     acc, _ = model_trim.test_accuracy(images, labels)
#     res = fusion_res.flatten().tolist()
#     model_trim.assign_weight(res)
#
#     for _ in range(100):
#         img, lab = d.train.generateSpecializedData_random_from_classes(class_list, 1200)
#         model_trim.train_model(img, lab)
#
#     acc_tune, _ = model_trim.test_accuracy(images, labels)
#     loss_value = acc - acc_tune
#     loss_res = torch.tensor(loss_value, requires_grad=True)
#     return loss_value
#
#
...
#         self.para = nn.Parameter(torch.ones(12416, 3), requires_grad=True)
#
#         init.constant_(self.fc.weight, val=1.0)
#         init.constant_(self.para, val=1.0)
#
#         self.class_list = class_list
#         self.optimizer = optim.Adam(self.parameters(), lr=0.1)
#
#     def forward(self, x):
#         y = torch.mul(x, self.para)
#         y = self.fc(y)
#         return y
#
...
#             #     print(param.data)
#             loss = loss_(y_pred, class_list=self.class_list)
#             loss.backward()
#             self.optimizer.step()
#             print('Epoch:', i, 'Loss:', loss.item())
#
#     def fitness(self, y_pred, class_list):
#         # Calculate fitness function for each individual
#         fitness_values = []
#         for y in y_pred:
#             # Use y as input to calculate fitness
#             # Here, you can modify your loss_ function to act as the fitness function
#             fitness_value = loss_(y, class_list=class_list)
...
#
#     def save_model(self):
#         model_path = './other/Fusion' + ('_'.join(str(x) for x in self.class_list)) + '.pth'
#         torch.save(self.state_dict(), model_path)
#         print('Model saved to:', model_path)
#
#     def load_model(self, model_path='./other/Fusion6_0_1.pth'):
...
#     print(input.shape)
#
#     fusion_net = CDRP_Fusion(class_list=indexes)
#
#     fusion_net.train(input)
#
#     fusion_net.save_model()
#
#
...
#     # 生成population_size个bits位的二进制字符串
#     population = []
...
#
#
# def genetic_algorithm(net, data, population_size, bits, mutation_rate, max_iterations):
#     population = generate_population(population_size, bits)  # 生成初始族群
#     for i in range(max_iterations):
#         fitnesses = [fitness_function(net, data) for _ in range(population_size)]  # 计算族群中每一个个体的适应度
#         best_individual = population[fitnesses.index(max(fitnesses))]  # 找到适应度最高的个体
