...
class FineTuneModel():
    ...
    def test_accuracy(self, test_images, test_labels):
        ys_pred_argmax, ys_true_argmax = self.sess.run(
            [self.lab_pre, self.lab_orin], feed_dict={
                self.xs: test_images,
    ...
    def mask_unit_by_value(self, classid):
        formulizedDict = {}
        json_path = "./ClassEncoding_cmq/class" + str(classid) + ".json"
    ...
    def build_model(self, graph, label_count=100):
        with graph.as_default():
            ...
            cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                labels=self.ys_orig, logits=self.ys_pred
            ))
            ...
            '''
            self.lab_pre = tf.argmax(self.ys_pred_softmax, 1)
            self.lab_orin = tf.argmax(self.ys_orig, 1)
            correct_prediction = tf.equal(tf.argmax(self.ys_orig, 1), tf.argmax(self.ys_pred_softmax, 1))
